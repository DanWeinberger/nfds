...
easyNF <- function(nf.obj    ){
  ...
  e_l <- apply(nf.obj$gene.mat.pre, 2,mean, na.rm=T)
  ...
  st.gps.nvt.pre<- as.matrix(table(sample.serotypes[pcvst.ind==0 & nf.obj$sample.year==2001], sample.GPSCs[pcvst.ind==0 & nf.obj$sample.year==2001]))
  st.prop.gps <- apply(st.gps.nvt.pre,2, function(x) x/sum(x))
  
   names(prev.gps) <- sort(as.numeric(dimnames(st.gps.nvt)[[2]] ))
  
  pred_post_serotype_prev <- round(st.prop.gps %*% matrix(prev.gps[dimnames(st.prop.gps)[[2]]], ncol=1),2)
