...
gene.name.int <- gene_name[cog_freq>0.05 & cog_freq<0.95]
...
d4.gamb.pre <- d4[d4$Country.x=='South Africa' & d4$`Clinical Manifest`=='Carriage' & d4$Vaccine_Period=='Pre-PCV',]
d4.gamb.post <- d4[d4$Country.x=='South Africa' & d4$`Clinical Manifest`=='Carriage' & d4$Vaccine_Period=='Post-PCV7',]
...
serotypes.period2 <-d4$In_Silico_Serotype[d4$Country.x==country & d4$`Clinical Manifest`=='Carriage' & d4$Vaccine_Period=='Post-PCV13']
#set.vts <- c('4','6A','6B','9V','14','18C','19F','23F')
set.vts <- c('1','4','5','6A','6B','7F','9V','14','18C','19A','19F','23F')

set.rr <-0.5

nf.obj <- list('sample.GPSCs'=sample.GPSCs,'sample.serotypes'=sample.serotypes,'gene.mat'=gene.mat, 'set.vts'=set.vts,'set.rr'=set.rr, 'post.serotypes'=serotypes.period2, country='South Africa')
